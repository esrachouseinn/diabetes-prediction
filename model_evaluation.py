...
plt.savefig("outcome_distribution.png")  # Save the outcome distribution plot
plt.show()
...
for idx, column in enumerate(diabetes_dataset.columns[:-1], 1):
    ...
    plt.savefig(f"{column}_distribution_by_outcome.png")  # Save each feature distribution plot
plt.tight_layout()
plt.show()
...
plt.savefig("correlation_matrix.png")  # Save the correlation matrix
plt.show()
...
for model_name, model in models.items():
    ...
    # Metrics for performance DataFrame
    ...
    plt.savefig(f'{model_name}_confusion_matrix.png')  # Save confusion matrix for each model
    plt.show()

# Save performance metrics as DataFrame and CSV
